#!/bin/bash
# OmniAvatar 14B V2V training with mask_all_frames + no_first_frame_overwrite + ref_sequence.
# Resumes from a trained checkpoint (takes path as argument).
#
# Usage: bash scripts/train_v2v_auxloss_4gpu_allmasked_refseq.sh /path/to/step-XXXX.pt
#
...
if [ -z "$1" ]; then
    ...
    echo "Example: $0 /home/work/output_omniavatar_v2v_maskall/step-2000.pt"
    exit 1
fi
...
CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch --config_file configs/accelerate_4gpus.yaml \
    ...
    --text_drop_prob 0.1 \
    --audio_drop_prob 0.1 \
    --output_path /home/work/output_omniavatar_v2v_maskall_refseq \
    --save_steps 500 \
    --use_sync_loss \
    --use_lpips_loss \
    --use_trepa_loss \
    ...
    --wandb_run_name "v2v_14B_maskall_refseq_from${STEP}" \
    --wandb_log_every 1 \
