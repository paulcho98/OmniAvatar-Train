#!/bin/bash
# Inference + metrics + stitching for mask_all_frames checkpoints.
# Runs 2 checkpoints × 3 datasets, optimized GPU scheduling.
set -euo pipefail
...
run_inference() {
    ...
    if [ -d "$comp_dir" ] && [ "$(ls "$comp_dir"/*.mp4 2>/dev/null | wc -l)" -gt 0 ]; then
        echo "  [SKIP] step-${step}/${ds} (composited exists)"
        return 0
    fi

    local hp="exp_path=${EXP_PATH},ckpt_name=step-${step}.pt,num_steps=${NUM_STEPS},guidance_scale=${GUIDANCE_SCALE},output_dir=${out_dir},latentsync_inference=true,use_mouth_only_compositing=true,face_detection_cache_dir=${face_cache},mask_all_frames=true,no_first_frame_overwrite=true"
    ...
    echo "  [DONE] step-${step}/${ds}: ${moved} videos"
}
...
# Schedule: for each checkpoint, run all 3 datasets on separate GPUs.
# For 2 checkpoints: Phase 1 = long datasets (4 GPUs), Phase 2 = short (2 GPUs).
# For 1 checkpoint: all 3 datasets on 3 GPUs simultaneously.
if [ "${#STEPS[@]}" -eq 1 ]; then
...
echo ""
echo "=== Metrics Summary (maskall, all checkpoints) ==="

# Collect all step directories that have metrics
ALL_STEPS=()
...
for step in "${ALL_STEPS[@]}"; do
    for ds in "${DATASETS[@]}"; do
        ...
        printf "%-10s %-14s %8s %8s %8s %8s %8s %8s %8s\n" \
            "step-${step}" "$ds" "${fvd:--}" "${fid:--}" "${csim:--}" "${ssim:--}" "${lmd:--}" "${syncd:--}" "${syncc:--}"
...
exec bash /home/work/.local/OmniAvatar/scripts/resume_v2v_allmasked_4gpus.sh
