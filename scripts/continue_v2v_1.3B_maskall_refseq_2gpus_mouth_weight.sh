#!/bin/bash
# Continue 1.3B V2V training: maskall → maskall + refseq + mouth_weight.
# 2-GPU warm restart from latest 1.3B maskall checkpoint (fresh optimizer, fresh wandb).
# Uses trainable_weights.pt from the latest checkpoint dir (auto-detected).
#
# === 1.3B Training Lineage ===
# Run 2: 1.3B_maskall_refseq_mouth_weight_2gpu (this script)
#   Source: 1.3B_maskall/checkpoint-2500/trainable_weights.pt (run 1, 2500 steps)
#   Output: /home/work/output_omniavatar_v2v_1.3B_maskall_refseq_mouth_weight_2gpu/ (1070 steps, killed)
#   Changes: added --use_ref_sequence (49ch→65ch), --mouth_weight 2.0, aux_sync 0.1→0.05,
#            aux_lpips 0.1→0.15, 4GPU→2GPU, grad_accum 2→4

SOURCE_DIR=/home/work/output_omniavatar_v2v_1.3B_maskall

# Auto-detect latest checkpoint
LATEST_CKPT=$(ls -td "$SOURCE_DIR"/checkpoint-* 2>/dev/null | head -1)
if [ -z "$LATEST_CKPT" ]; then
    echo "ERROR: No checkpoint found in $SOURCE_DIR"
    exit 1
fi
...
echo "Starting from $CKPT_FILE (step $STEP)"
...
CUDA_VISIBLE_DEVICES=0,1 accelerate launch --config_file configs/accelerate_2gpus.yaml \
    ...
    --text_drop_prob 0.1 \
    --audio_drop_prob 0.1 \
    --output_path /home/work/output_omniavatar_v2v_1.3B_maskall_refseq_mouth_weight_2gpu \
    --save_steps 500 \
    --use_sync_loss \
    --use_lpips_loss \
    --use_trepa_loss \
    ...
    --wandb_run_name "v2v_1.3B_maskall_refseq_mouth_weight_2gpu_from${STEP}" \
    --wandb_log_every 1 \
