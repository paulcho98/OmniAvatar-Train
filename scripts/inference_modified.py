...
class WanInferencePipeline(nn.Module):
    ...
    def forward(self, prompt, 
        ...
        if audio_path is not None and args.use_audio:
            ...
            # padding audio (no zero prefix — chunk 0 takes L frames directly)
            if audio_len < L:
                audio_len = audio_len + (L - audio_len % L) if audio_len % L != 0 else audio_len
            elif (audio_len - L) % (L - fixed_frame) != 0:
                audio_len = audio_len + ((L - fixed_frame) - (audio_len - L) % (L - fixed_frame))
            ...
            audio_prefix = audio_embeddings[-fixed_frame:]  # No zero prefix — use real audio for carry-forward
        else:
            audio_embeddings = None

        # loop (no zero prefix — chunk 0 takes L frames)
        times = 1
        if seq_len > L:
            times += math.ceil((seq_len - L) / (L - fixed_frame))
        video = []
        image_emb = {}
        img_lat = None
        if args.i2v:
        ...
        for t in range(times):
            ...
            if audio_embeddings is not None:
                if t == 0:
                    # No zero prefix — take L frames directly (matches training alignment)
                    audio_tensor = audio_embeddings[
                            :min(L, audio_embeddings.shape[0])
                        ]
                else:
                    audio_start = L + (t - 1) * (L - overlap)
                    audio_tensor = audio_embeddings[
        ...
        video = video[:, :ori_audio_len]  # No +1: frame 0 has real audio, not silence
        return video
